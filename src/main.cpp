...
//       LLVMGetGlobalContext(), (LLVMTypeRef[]){LLVMInt32Type(), LLVMInt32Type()},
//       2, false);
...
//   // 创建指向结构体类型的指针的指针
//   LLVMTypeRef structPointerPointerType = LLVMPointerType(structPointerType, 0);

//   // 分配内存并加载指针
//   LLVMValueRef value = LLVMGetParam(strxFunc, 0); // 指向结构体类型的指针的指针
//   LLVMValueRef allocaInst = LLVMBuildAlloca(
//       builderRef, structPointerPointerType, "myStructPointerPointer");
//   LLVMBuildStore(builderRef, value, allocaInst); // 将指针存储到分配的内存中
...
//   auto retType = LLVMStructTypeInContext(LLVMGetGlobalContext(), agg, 2, false);
//   LLVMTypeRef oFun = LLVMFunctionType(retType, (LLVMTypeRef[]){int32Type, int32Type}, 2,false);
//   LLVMAddFunction(Module, "llvm.sadd.with.overflow.i32",oFun);
//   LLVMTypeRef funcType = LLVMFunctionType(int32Type, (LLVMTypeRef[]){int32Type,int32Type}, 2, false);
//   LLVMValueRef func = LLVMAddFunction(Module, "switch_test", funcType);

//   LLVMBasicBlockRef entry = LLVMAppendBasicBlockInContext(LLVMGetGlobalContext(), func, "entry");
//   LLVMBasicBlockRef zero_case = LLVMAppendBasicBlockInContext(LLVMGetGlobalContext(), func, "zero_case");
//   LLVMBasicBlockRef one_case = LLVMAppendBasicBlockInContext(LLVMGetGlobalContext(), func, "one_case");
//   LLVMBasicBlockRef default_case = LLVMAppendBasicBlockInContext(LLVMGetGlobalContext(), func, "default_case");
...
//   LLVMValueRef is_zero = LLVMBuildICmp(builder, LLVMIntEQ, arg, LLVMConstInt(int32Type, 0, false), "is_zero");
...
//   LLVMValueRef load_result = LLVMBuildLoad2(builder,int32Type, result, "load_result");
//   LLVMValueRef switchInst = LLVMBuildSwitch(builder, load_result, default_case, 2);
...
int main() {
  ...
  LLVMValueRef printf_arg1 = LLVMBuildGlobalStringPtr(builder, "Hello, %s!\n", "printf_arg1");

  LLVMValueRef printf_args[] = { printf_arg1 };
  
  LLVMValueRef printf_result = LLVMBuildCall2(builder,printf_type, printf_func, printf_args, 1, "printf");
  ...
  // int (*main_func_ptr)() = (int (*)())LLVMGetFunctionAddress(engine, "main");
